  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] bg-white overflow-hidden">
      ...
      <header className="flex-none h-14 border-b border-lectra-border bg-white px-6 flex items-center justify-between z-10">
         <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-xs text-lectra-text-secondary">
                <Link href={`/schools/${schoolID}`} className="hover:text-lectra-primary transition-colors">{schoolName}</Link>
                <span>/</span>
            </div>
            <h1 className="text-lg font-extrabold text-lectra-text tracking-tight truncate max-w-[300px]">{courseName}</h1>
            <span className="bg-lectra-primary/10 text-lectra-primary text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">{courseID}</span>
         </div>

         <div className="flex items-center gap-6">
            <div className="hidden lg:flex items-center gap-4 text-xs text-lectra-text-secondary border-r border-lectra-border pr-6">
                <span className="flex items-center gap-1">👥 <b>2.3k</b> Students</span>
                <span className="flex items-center gap-1">📚 <b>142</b> Resources</span>
            </div>
            <div className="flex gap-3">
                <button className="text-xs font-bold text-lectra-text hover:text-lectra-primary transition-colors flex items-center gap-1">
                  <span>⭐</span> Save
                </button>
                <button className="text-xs font-bold text-lectra-text hover:text-lectra-primary transition-colors flex items-center gap-1">
                  <span>📤</span> Share
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 border-r border-lectra-border bg-lectra-surface flex flex-col min-w-[350px] relative">
            {/* Chat Header */}
                ...
                <div className="flex gap-3">
                  ...
                  <div className="bg-white border border-lectra-border rounded-2xl rounded-tl-none px-4 py-3 flex-1 shadow-sm max-w-[85%]">
                    <p className="text-lectra-text text-sm leading-relaxed">
                      Hello! I&apos;m ready to help you with <strong>{courseName}</strong>. Need help finding a quiz or understanding a topic?
                    </p>
                  </div>
                </div>
            ...
            <div className="p-4 border-t border-lectra-border bg-white">
                <div className="relative">
                    ...
                    />
                    <button className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-lectra-accent text-white rounded-lg flex items-center justify-center hover:bg-lectra-accent-hover transition-all">
                        ↑
                    </button>
                </div>
            </div>
        ...
        <div className="w-1/2 flex flex-col bg-white min-w-[350px]">
            {/* Tabs Header */}
            <div className="flex border-b border-lectra-border bg-white sticky top-0 z-10">
                <button 
                    onClick={() => setActiveTab('materials')}
                    className={`flex-1 py-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'materials' ? 'border-lectra-primary text-lectra-text bg-lectra-surface/50' : 'border-transparent text-lectra-text-secondary hover:text-lectra-text hover:bg-gray-50'}`}
                >
                    📚 Course Materials
                </button>
                <button 
                    onClick={() => setActiveTab('upload')}
                    className={`flex-1 py-4 text-sm font-bold transition-all border-b-2 ${activeTab === 'upload' ? 'border-lectra-primary text-lectra-text bg-lectra-surface/50' : 'border-transparent text-lectra-text-secondary hover:text-lectra-text hover:bg-gray-50'}`}
                >
                    📤 Upload & Share
                </button>
            </div>
